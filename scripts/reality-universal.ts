/**
 * Shor's Algorithm Order Finding — Universal Reality System (URS v1.0) Execution Engine
 ...
 * Gate 4: Shor Order Finding Mathematical Invariants & Factorization
 ...
 * Gate 8: Qiskit Circuit Structure & Inverse QFT Conformance
...
import {
  ...
  findPeriod,
  factorizeWithShor
} from '../src/shor_order_finding.js';
...
console.log('╔══════════════════════════════════════════════════════════════════════════╗');
console.log('║   SHOR ORDER FINDING — UNIVERSAL REALITY SYSTEM (URS v1.0)               ║');
...
try {
  ...
  assert.strictEqual(manifest.system, 'SHORS-ALGORITHM-ORDER-FINDING');
...
// -----------------------------------------------------------------------------
try {
  const seed = new Uint8Array(32).fill(0x6d);
...
try {
  // Test gcd
  assert.strictEqual(gcd(21, 14), 7);
  // Test modPow
  assert.strictEqual(modPow(7, 4, 15), 1);
  // Test findPeriod for a=7, N=15
  const r = findPeriod(7, 15);
  assert.strictEqual(r, 4, 'Period of 7 mod 15 must be 4');
  // Test factorizeWithShor
  const result = factorizeWithShor(15, 7);
  assert.strictEqual(result.verified, true);
  assert.strictEqual(result.p * result.q, 15);
  assert.ok((result.p === 3 && result.q === 5) || (result.p === 5 && result.q === 3));

  gates.push({
    gate: 4,
    name: 'Shor Order Finding Mathematical Invariants & Factorization',
    passed: true,
    score: 1.0,
    details: 'Verified Shor order finding: a=7, N=15 -> r=4 -> factors p=3, q=5'
  });
  console.log('▶ [URS GATE 4/10] Shor Order Finding Mathematical Invariants & Factorization');
  console.log('  ✅ Verified Shor order finding: a=7, N=15 -> r=4 -> factors p=3, q=5\n');
} catch (e: any) {
  gates.push({ gate: 4, name: 'Shor Order Finding Mathematical Invariants & Factorization', passed: false, score: 0.0, details: e.message });
...
try {
  ...
  const sigResult = createPqcHybridSignature('SHOR_PROOF_GATE5', keyPair, 0.05, 'shor-order-finding');
  assert.ok(sigResult.hybridSignature.startsWith('PQC-HYBRID-x402.'));

  const ver = verifyPqcSignature(sigResult.hybridSignature, 'SHOR_PROOF_GATE5', keyPair.publicKey, 0.05, 'shor-order-finding');
  ...
  const verTampered = verifyPqcSignature(tamperedSig, 'SHOR_PROOF_GATE5', keyPair.publicKey, 0.05, 'shor-order-finding');
...
try {
  const keyPair = generatePqcKeyPair('ML-DSA-65');
  const sigResult = createPqcHybridSignature('SHOR_SETTLEMENT', keyPair, 0.1, 'shor-service');
...
try {
  ...
  const rawPair = ml_kem768.keygen(new Uint8Array(64).fill(0x7c));
...
// -----------------------------------------------------------------------------
try {
  const mainPyPath = path.resolve('main.py');
  assert.ok(fs.existsSync(mainPyPath), 'main.py must exist');
  const mainContent = fs.readFileSync(mainPyPath, 'utf8');
  assert.ok(mainContent.includes('QuantumCircuit'), 'Must use QuantumCircuit');
  assert.ok(mainContent.includes('QFT'), 'Must use QFT inverse library');
  assert.ok(mainContent.includes('run_order_finding_subroutine_demo'), 'Must define order finding subroutine');

  gates.push({
    gate: 8,
    name: 'Qiskit Circuit Structure & Inverse QFT Conformance',
    passed: true,
    score: 1.0,
    details: 'Verified Qiskit circuit construction, Hadamard superposition, controlled-U phase, and Inverse QFT'
  });
  console.log('▶ [URS GATE 8/10] Qiskit Circuit Structure & Inverse QFT Conformance');
  console.log('  ✅ Verified Qiskit circuit construction, Hadamard superposition, controlled-U phase, and Inverse QFT\n');
} catch (e: any) {
  gates.push({ gate: 8, name: 'Qiskit Circuit Structure & Inverse QFT Conformance', passed: false, score: 0.0, details: e.message });
